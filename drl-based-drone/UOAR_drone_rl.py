...
class DroneEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # Track fires seen by any drone this step (union across drones)
        fires_seen_union = set()
        per_drone_visible = []
        # Precompute 2D fire centers array for occlusion tests (or discretized line/wall)
        ...
        for i in range(self.n_drones):
            ...
            z = pos[2]
            view_radius = k * z / 8
            visible_list = []  # list of tuples (idx, visible_point_x, y, z)

            # Evaluate each fire center to determine if it's visible (not occluded) and within view
            for idx, fc in enumerate(self.fire_centers):
                fc_xy = np.array([fc[0], fc[1]])
                v_target = fc_xy - drone_xy
                dist_target = np.linalg.norm(v_target)
                if dist_target <= (view_radius + self.fire_radius):
                    # Determine whether any other fire blocks the line-of-sight to this fire
                    blocked_by = None
                    for jdx, fb in enumerate(self.fire_centers):
                        if jdx == idx:
                            continue
                        ...
                        if dist_fire + 1e-6 < dist_target:
                            # Check angular alignment (nearly colinear) and whether fb covers the ray
                            if dist_fire > 1e-6 and dist_target > 1e-6:
                                cos_angle = np.dot(v_fire, v_target) / (dist_fire * dist_target + 1e-8)
                                if cos_angle > 0.99:
                                    # Check perpendicular distance from fb center to the ray
                    ...
                    if blocked_by is None:
                        # Unoccluded: visible point is the perimeter point on fire facing the drone
                        if dist_target > 1e-6:
                            unit = (fc_xy - drone_xy) / dist_target
                            vis_xy = fc_xy - unit * min(self.fire_radius, dist_target - 1e-6)
                        else:
                            vis_xy = fc_xy.copy()
                        vis_z = fc[2]
                        visible_list.append((idx, vis_xy[0], vis_xy[1], vis_z))
                        fires_seen_union.add(idx)
                    else:
                        # Occluded: compute visible edge point on the occluding fire along the ray
                        jdx, fb = blocked_by
                        fb_xy = np.array([fb[0], fb[1]])
                        dist_block = np.linalg.norm(fb_xy - drone_xy)
                        if dist_block > 1e-6:
                            unit = (fc_xy - drone_xy) / (dist_target + 1e-8)
                            # point where ray first intersects occluding fire perimeter
                            hit_xy = drone_xy + unit * max(0.0, dist_block - self.fire_radius)
                        else:
                            hit_xy = fb_xy.copy()
                        hit_z = fb[2]
                        visible_list.append((jdx, hit_xy[0], hit_xy[1], hit_z))
                        fires_seen_union.add(jdx)
            per_drone_visible.append((visible_list, pos, other))

        # Build per-drone observation vectors. Share union of visible fires across all drones
        # Convert union into a sorted list by index and trim/pad to max_visible_fires
        union_list = sorted(list(fires_seen_union))
        # For deterministic ordering, sort by proximity to drone 0 if needed (but keep index order)
        for d_idx in range(self.n_drones):
            visible_list, pos, other = per_drone_visible[d_idx]
            # Map index -> visible point for this drone (prefer unoccluded entry if present)
            vis_map = {v[0]: v[1:] for v in visible_list}
            base = np.concatenate([pos, [1.0 if visible_list else 0.0], other, np.array([0.0, 0.0, 0.0])])
            # Prepare visible fires block
            fires_block = []
            # Use union: if a fire was seen by any drone, include its visible point if available to this drone,
            # otherwise include zeros (or fallback to center clipped to perimeter toward this drone)
            for fid in union_list[:self.max_visible_fires]:
                if fid in vis_map:
                    fx, fy, fz = vis_map[fid]
                    fires_block.extend([fx, fy, fz, 1.0])
                else:
                    # Fallback: provide center projected to perimeter toward this drone
                    # compute vector from drone to fire center
                    fc = self.fire_centers[fid]
                    drone_xy = np.array([pos[0], pos[1]])
                    fc_xy = np.array([fc[0], fc[1]])
                    dist = np.linalg.norm(fc_xy - drone_xy)
                    if dist > 1e-6:
                        unit = (fc_xy - drone_xy) / dist
                        fx, fy = fc_xy - unit * min(self.fire_radius, dist - 1e-6)
                    else:
                        fx, fy = fc_xy
                    fires_block.extend([fx, fy, fc[2], 1.0])
            # pad remaining slots
...
=======
>>>>>>> 6467273d845edbe31fb8cf6d85733f1f2822cb46
...
>>>>>>> 6467273d845edbe31fb8cf6d85733f1f2822cb46
    ...
    # Remove interactive visualization prompt by defaulting to False (auto-play only on final scenario)
    show_vis_each = False
    ...
    for scenario_idx, scenario in enumerate(scenarios):
        ...
        for section in sections:
            for curriculum_level in range(5):  # 5 curriculum levels per scenario
                ...
                for episode in range(curriculum_episodes):
                    print(f"  Starting episode {episode+1}/{curriculum_episodes} for curriculum level {curriculum_level+1}")
                    obs, _ = env.reset()
        ...
        if last_episode_states is not None and (show_vis_each or is_last_scenario):
            print("Playing episode visualization (auto)...")
            # Ensure plots directory exists
            plots_dir = Path('plots')
            plots_dir.mkdir(exist_ok=True)
            # Save a mid-episode snapshot while playing
            ...
            for t in range(len(last_episode_states)):
                ...
                # After rendering, save the figure for a middle frame
                try:
    ...
    plt.subplot(1, 3, 1)
    # If multiple training runs are present in logs (training_metrics_*.csv), show mean +/- std
    try:
        log_files = sorted(Path('logs').glob('training_metrics*.csv'))
        if len(log_files) > 1:
            runs = []
            for f in log_files:
                df = None
                try:
                    ...
                    continue
                runs.append(df['total_reward'].to_numpy())
            if runs:
                # pad to equal length with NaN, compute mean/std ignoring NaN
                ...
                if mean.size > 0:
                    plt.plot(x, mean, label='Mean Total Reward', color='C0')
                    plt.fill_between(x, mean - std, mean + std, color='C0', alpha=0.2, label='Std Dev')
            else:
    ...
    if hasattr(plt, 'savefig') and hasattr(env, 'cell_rewards'):
        ...
        exp_vmin = float(np.nanmin(env.cell_rewards))
        exp_vmax = float(np.nanmax(env.cell_rewards))
